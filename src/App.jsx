...
export default function RobotOyini() {
  ...
  function optimizeCommand(command) {
    return command.replace(/(.)\1+/g, (match, p1) => match.length + p1);
  }

  function buyruqlarniBajar() {
    const buyruqlar = buyruq.toUpperCase();
    const optimized = optimizeCommand(buyruqlar);
    const timestamp = new Date().toLocaleString();
    setBuyruqlarRuyxati([
      ...buyruqlarRuyxati,
      { original: buyruqlar, optimized, timestamp },
    ]);
  ...
  return (
    <div className="flex flex-col items-center gap-5 p-5">
      ...
      <div className="buyruqlar-ruyxati w-full max-w-[400px]">
        <h3 className="font-bold text-2xl pb-5">Command History</h3>
        <table className="w-full text-left border-collapse">
          <thead>
            <tr>
              <th className="border-b py-2">Original</th>
              <th className="border-b py-2">Optimized</th>
              <th className="border-b py-2">TimeStamp</th>
            </tr>
          </thead>
          <tbody>
            {buyruqlarRuyxati.map((entry, index) => (
              <tr key={index}>
                <td className="border-b py-2">{entry.original}</td>
                <td className="border-b py-2">{entry.optimized}</td>
                <td className="border-b py-2">{entry.timestamp}</td>
              </tr>
            ))}
          </tbody>
        </table>
